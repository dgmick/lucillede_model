...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_post_masonry'); ?> data-category="<?php echo esc_attr($post_categs); ?>">
	...
	<?php
		...
		if (!post_password_required() && $cmsmasters_post_image_link != '') {
			top_model_thumb(get_the_ID(), 'cmsmasters-project-masonry-thumb', false, 'img_' . get_the_ID(), true, true, true, true, $cmsmasters_post_image_link);
		} elseif (!post_password_required() && has_post_thumbnail()) {
			top_model_thumb(get_the_ID(), 'cmsmasters-project-masonry-thumb', false, 'img_' . get_the_ID(), true, true, true, true, false);
		}
