...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_project_puzzle ' . $cmsmasters_project_size); echo (($project_categs != '') ? ' data-category="' . esc_attr($project_categs) . '"' : '') ?>>
	...
	<?php 
		top_model_thumb_rollover(get_the_ID(), 'full', false, $rollover, false, false, false, false, false, false, true, $cmsmasters_project_link_redirect, $cmsmasters_project_link_url, $cmsmasters_project_link_target, 'cmsmasters_theme_icon_image', 'portfolio');
				...
				if ($title || $categories) {
					echo '<div class="project_inner_middle">' . 
						'<div class="project_inner_table">' . 
							...
							$title ? top_model_project_heading(get_the_ID(), 'h3', $cmsmasters_project_link_redirect, $cmsmasters_project_link_url, $cmsmasters_project_link_target) : '';
		...
		if (!$title) {
			echo '<div class="dn">';
				top_model_project_heading(get_the_ID(), 'h3');
			echo '</div>';
		}
